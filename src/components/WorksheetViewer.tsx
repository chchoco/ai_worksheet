      ...
      <div className="p-3 sm:p-6 flex-1 flex flex-col items-center justify-start">
        ...
        {activeTab === 'pdf' && (
          <div
            ...
            className="w-full max-w-5xl flex flex-col items-center gap-4 transition-transform duration-150 origin-top"
            ...
            <div className="w-full bg-white rounded-2xl shadow-lg border border-slate-200 overflow-hidden flex flex-col">
              ...
              <div className="bg-slate-800 text-white px-4 py-2.5 flex items-center justify-between text-xs">
                <div className="flex items-center gap-2">
                  ...
                  <span className="font-bold truncate max-w-xs sm:max-w-md">{worksheet.pdfFileName}</span>
                </div>
                <div className="flex items-center gap-3 text-slate-300">
                  <span>{formatBytes(worksheet.fileSizeBytes)}</span>
                  <button
                    onClick={handleOpenNewTab}
                    className="text-indigo-300 hover:text-white flex items-center gap-1 font-semibold"
                  >
              ...
              {/* PDF Renderer */}
              {pdfSourceUrl ? (
                <div className="w-full h-[750px] sm:h-[880px] lg:h-[980px] bg-slate-200">
                  <object
                    data={`${pdfSourceUrl}#toolbar=1&navpanes=0&view=FitH`}
                    type="application/pdf"
                    className="w-full h-full"
                  >
                    <iframe
                      src={`${pdfSourceUrl}#toolbar=1&navpanes=0&view=FitH`}
                      title={worksheet.title}
                      className="w-full h-full border-0"
                    >
                      {/* Fallback for browsers that cannot embed */}
                      <div className="p-8 text-center bg-white h-full flex flex-col items-center justify-center">
                        ...
                        <h4 className="text-base font-bold text-slate-800 mb-1">
                          {worksheet.pdfFileName}
                        </h4>
                        <p className="text-xs text-slate-500 mb-4">
                          브라우저 설정으로 인해 인라인 미리보기가 지원되지 않을 경우 아래 버튼으로 확인하실 수 있습니다.
                        </p>
                ...
                <div className="w-full h-[400px] flex flex-col items-center justify-center text-slate-400">
                  ...
                  <p className="text-sm font-semibold">등록된 PDF 파일이 없습니다.</p>
                </div>
              )}
            </div>
